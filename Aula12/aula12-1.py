...
def desenhar_cenario(ax,matriz_trans,titulo,cor_trans,label_trans,matriz_vertices):
    ...
    ax.annotate(f"A({x_trans[0]:.1f},{y_trans[0]:.1f})",(x_trans[0],y_trans[0]),xytext=(5,5),
...
def plotar_reflexao():
    print("=== Reflexão ===\n")
    # Geometria Original
    # Triângulo: A(2,2), B(4,5), C(6,2) e fechamento com A(2,2)
    ...
    # Reflexão no Eixo XY (Inverte ambos)
    M_RefXY = np.array([
        [-1,0,0],
        [0,-1,0],
        [0,0,1]
    ...
    desenhar_cenario(axs[1,0],trans_XY,"3) Reflexão no Eixo XY (Origem)",
                        'purple','Refletido XY',matriz_vertices)    
    desenhar_cenario(axs[1,1],trans_Rot,"4) Rotação de 60° na Origem",
                        'orange','Rotacionado 60°',matriz_vertices)
    plt.tight_layout()
