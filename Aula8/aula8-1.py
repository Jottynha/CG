...
def transformacao_translacao():
    print("=== Transformação da Translação ===")
    # Definindo da Geometria Original (Coordenadas Homogêneas)
    # Vértices: A(1,3), B(3,7), C(5,3) e A(1,3) para fecharo polígono
    ...
    # Vamos transladar o triângulo 2 unidades para a direita (+X) e 2 unidade para cima (+Y)
    tx = 2
    ty = 2
    ...
    # Renderização 2D com Matplotlib
    ...
    # Rótulos A,B,C para os pontos dos triângulos
    rotulos = ['A','B','C']
    for i in range(3):
        # original
        ax.annotate(f"{rotulos[i]} {x_orig[i]:.0f},{y_orig[i]:.0f}",(x_orig[i],y_orig[i]),textcoords="offset points", xytext=(-15,-15), color='blue')
        # transladado
        ax.annotate(f"{rotulos[i]}' {x_trans[i]:.0f},{y_trans[i]:.0f}",(x_trans[i],y_trans[i]),textcoords="offset points", xytext=(-15,-15), color='darkred')
