...
def simulador_pipeline_grafico():
    ...
    # SRO (Objeto)
    # Um vértice do Homem (personagem). O centro do Homem é (0,0)
    # O Homem está 5 unidades à frente e 2 acima do eixo
    # (X,Y,W) W (Weight ou Peso Homogêneo) 
    # funciona como uma "flag", que ensina a GPU a diferença entre um Ponto e um Vetor
    P_SRO = np.array([[5.0],[2.0],[1.0]])
    imprimir_ponto("1. SRO (Objeto)",P_SRO)

    # SRU (Universo)
    # Colocar o Homem na posição (100,50) no mundo virtual
    Matriz_Model = np.array([[1,0,100],[0,1,50],[0,0,1]])
